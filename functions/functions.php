		...
		if (mysqli_num_rows($run_check_pro) > 0) {
			...
			if ($cart_SoLuong <= $soLuongKho) {
				...
				echo "
					...
					window.open('allproducts.php','_self');
					</script>
					";
			}
		} else {
	...
	while ($row_carts = mysqli_fetch_array($run_carts)) {
		...
		echo "
					...
					<div class='produc-shopping-left'>
						<img src='/webquanly/admin/product_images/$cart_Hinh' alt='$cart_Ten'>
					</div>
...
function update_cart_del($product_id)
{
	...
	if ($fetch_pro) {
		...
		<script>window.open(webquanly/cart.php,'_self')</script>
		";
	}
}
...
function getCats()
{
	...
	while ($row_cats = mysqli_fetch_array($run_cats)) {
		...
		echo "
				<li><a href='allproducts.php?cat=$cat_id'>$cat_title</a></li>
				";
	}
}
				...
				<div class='col-md-4 mt-3'>
    		    	<div class='card profile-card-5'>
    		        	<div class='card-img-block'>
    		            	<img class='card-img-top' src='/webquanly/admin/product_images/$pro_image' alt='$pro_title'>
    		        	</div>
                    	<div class='card-body pt-0'>
                    		<h5 class='card-title'>$pro_title</h5>
							<span class='price-new'>Giá $pro_price</span> <del class='price-old'></del>
						 
							<a href='allproducts.php?add_cart=$pro_id' class='btn btn-sm btn-primary float-right'>Mua ngay</a>	
